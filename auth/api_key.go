...
func NewApiKey(guard string) contractauth.Auth {
	return &ApiKey{
		guard: guard,
	}
}

func (app *ApiKey) Guard(name string) contractauth.Auth {
	return NewAuth(name)
}

// User need parse token first.
func (app *ApiKey) User(ctx *frame.Context, user any) error {
...
func (app *ApiKey) Parse(ctx *frame.Context, token string) error {
...
func (app *ApiKey) Login(ctx *frame.Context, user any) (token string, err error) {
...
func (app *ApiKey) LoginUsingID(ctx *frame.Context, id any) (token string, err error) {
	jwtSecret := facades.Config.GetString("jwt.secret")
	if jwtSecret == "" {
...
func (app *ApiKey) Refresh(ctx *frame.Context) (token string, err error) {
	auth, ok := ctx.Value(ctxKey).(Auth)
	if !ok || auth[app.guard] == nil {
		return "", ErrorParseTokenFirst
	}
...
func (app *ApiKey) Logout(ctx *frame.Context) error {
	auth, ok := ctx.Value(ctxKey).(Auth)
...
func (app *ApiKey) makeAuthContext(ctx *frame.Context, claims *Claims, token string) {
	ctx.Set(ctxKey, Auth{
